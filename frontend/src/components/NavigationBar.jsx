	...
	return (
		<header className='flex flex-col md:flex-row justify-between items-center p-4 bg-gray-100'>
			...
			<nav className='flex items-center space-x-4'>
				<AuthorizedOnly>
					<LogoutButton />
					<Link to="/myprofile" className='hover:opacity-50 transition-opacity'>Мой профиль</Link>
				</AuthorizedOnly>
				<UnauthorizedOnly>
					<Link to='/login' className='hover:opacity-50 transition-opacity'>Войти</Link>
				</UnauthorizedOnly>
				<AdminOnly>
					<Link to="/admin" className='hover:opacity-50 transition-opacity'>Админ панель</Link>
				</AdminOnly>
				<Link to="/about" className='hover:opacity-50 transition-opacity'>О нас</Link>
				<Link to="/job" className='hover:opacity-50 transition-opacity'>Заказы</Link>
			</nav>
		</header>
	);
}
